...
export default async function handler(req, res) {
  ...
  if (!session) {
    ...
    try {
      ...
      const document = await client.db('demostrator-forms').collection('form1').findOne({ _id: data._id });
      if (document) {
        res.status(409).end();
      } else {
        const collection = await client.db('demostrator-forms').collection('form1');
